...
3. Close the remaining startup/reload/runtime gaps in `src/dnsmasq.rs` and `src/main.rs` — SIGHUP/API reload now really reaches the live forward loop (issue #174) and re-reads DHCP hosts/options/ethers files (#175), but making that DHCP reload reach an already-running `run_dhcp_loop` (#179) and `--conf-file` change-watching (#176) are still open.
