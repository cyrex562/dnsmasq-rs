...
All optional subsystems are gated behind Cargo features that mirror dnsmasq's compile-time `HAVE_*` defines. Default features: `dhcp dhcp6 dnssec auth tftp loop inotify dump`. Feature-gated code uses `#[cfg(feature = "...")]` on both `mod` declarations in `lib.rs`/`main.rs` and inside modules. There is no `bpf` feature — `src/bpf.rs` was speculative classic-BPF filter code with no upstream counterpart and no caller; it was removed. Upstream `bpf.c` is BSD/Solaris-only routing-socket code with no Linux relevance; its Linux analog is `netlink.rs`.
